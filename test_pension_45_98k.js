...
// Читаем JSON файл
const jsonPath = path.join(__dirname, 'test_pension_45_98k.json');
...
console.log('ТЕСТ РАСЧЕТА: ПЕНСИЯ');
console.log('='.repeat(80));
console.log('');
                ...
                if (parsed.results && parsed.results.length > 0) {
                    ...
                    if (result.error) {
                        ...
                        if (result.pension_details) {
                            ...
                            console.log('');
                            if (pd.current_age !== undefined) {
                                console.log('  Возраст клиента:', pd.current_age, 'лет');
                            }
                            if (pd.years_to_retirement !== undefined) {
                                console.log('  Лет до пенсии:', pd.years_to_retirement);
                            }
                            if (pd.projected_ipk !== undefined) {
                                console.log('  Прогнозируемый ИПК:', pd.projected_ipk.toLocaleString('ru-RU'));
                            }
                            if (pd.projected_fixed_payment !== undefined) {
                                console.log('  Прогнозируемая фиксированная выплата:', pd.projected_fixed_payment.toLocaleString('ru-RU'), 'руб/мес');
                            }
                            console.log('');
                        }
