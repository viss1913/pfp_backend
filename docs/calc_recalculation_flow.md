...
- constructor (`site/MAX/telegram`, `constructorAiService`);
- B2C `chat_AI` (`POST /my/ai-b2c/chat_AI/stream`, `aiB2cService`).

Переходим в режим пересчёта только если одновременно:

1. В текущем ходе классификатор выбрал команду `/calc` (или алиас `/recalc`, `/recalculate`).
2. Есть валидный `pfp_client_id` (в constructor — из `constructor_clients`, в B2C — `req.user.clientId`).
...
- Если пользователь явно назвал сумму ежемесячного пополнения/взноса, extraction обязан класть её в `goal_patch.monthly_replenishment`.
...
Для `chat_AI` ответ отправляется в SSE как:
- `type=pdf_url` (если ссылка есть),
- `type=text` (итоговый текст, с цифрами только из calc JSON),
- `type=done`.
