...
- `page-detailed-plan-v2.html` использует тот же расчёт строк, что v1 `buildRepleneshmentRows` в `buildFinamReportHtml.js`: полный `goals_detailed` передаётся в модель как `replenishmentReport`. Первый месяц — текущий: пополнение включает стартовый капитал по целям и годовую премию LIFE при наличии страхования жизни; далее помесячные строки с пополнениями, вычетами, софинансированием и капиталом. До 26 строк на два листа превью/PDF.
