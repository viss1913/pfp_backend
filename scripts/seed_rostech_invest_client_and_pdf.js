/**
 * Создаёт клиента Ростех (project 22): Александр, INVESTMENT 100k + 5k × 15 лет.
 * Считает ПФП, пишет goals_summary, генерирует PDF в tmp/ и печатает путь.
 *
 * Usage: node scripts/seed_rostech_invest_client_and_pdf.js
 */
const fs = require('fs');
...
async function main() {
    ...
    if (!agent) {
        console.warn(
            `[WARN] Нет агента с project_id=${ROSTECH_PROJECT_ID} — клиент будет без agent_id (в списке по агенту не виден).`
        );
    }

    const stamp = Date.now();
    const email = `rostech.invest.${stamp}@example.invalid`;
    ...
    console.log('Создание клиента + цель...');
    ...
    const clientForCalc = {
        ...
        birth_date: full.birth_date || '1985-01-01',
    };
    ...
    const inv = (calculation.goals || calculationResponse.goals || []).find(
        (g) => String(g.goal_type || '').toUpperCase() === 'INVESTMENT'
    );
    if (inv && inv.summary) {
        console.log('INVESTMENT summary:', {
            projected_capital_at_end: inv.summary.projected_capital_at_end,
            initial_capital: inv.summary.initial_capital,
            monthly_replenishment: inv.summary.monthly_replenishment,
        });
    }
    ...
    const pdfPath = path.join(outDir, `report-client-${clientId}-rostech-invest.pdf`);
    fs.writeFileSync(pdfPath, pdfBuffer);

    console.log('\n--- Готово ---');
    console.log('PDF:', pdfPath);
    console.log('Байт:', pdfBuffer.length);
    console.log('Email (уникальный):', email);
