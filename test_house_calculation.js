...
/**
 * Тестовый скрипт для расчета цели "Дом"
 * Использование: node test_house_calculation.js [railway-url]
 */

const railwayUrl = process.argv[2] || 'https://pfpbackend-production.up.railway.app';

// Запрос для расчета "Дом"
const testRequest = {
    goals: [
        {
            ...
            initial_capital: 200000    // 200 тыс рублей
        }
    ]
};
...
console.log('РАСЧЕТ ЦЕЛИ: ДОМ');
console.log('='.repeat(80));
console.log('');
console.log('Параметры:');
console.log('  • Стоимость: 10,000,000 руб.');
console.log('  • Срок: 5 лет (60 месяцев)');
console.log('  • Первоначальный капитал: 200,000 руб.');
console.log('');
console.log('─'.repeat(80));
                    ...
                    if (result.error) {
                        ...
                        if (result.financials) {
                            ...
                            if (fin.recommended_replenishment && fin.capital_gap) {
                                ...
                                console.log('  С учетом начального капитала:', (fin.initial_capital + totalReplenishment).toLocaleString('ru-RU'), 'руб.');
                                console.log('');
                            }
                        }
