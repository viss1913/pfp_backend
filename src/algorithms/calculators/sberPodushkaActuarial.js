/**
 * Актуарный расчёт «Страхование по подписке» (ex «Подушка безопасности») —
 * порт docs/partners/sber/Podushka final.py
 * Годовая премия prem_all_1, подписка prem_all_12 (k_12=1.06), округление до 5 ₽.
 * Тариф зависит от возраста / пола / срока — не фиксированные 1,44%.
 */
...
function buildSberPodushkaNsjResult(goal, client) {
    ...
    return {
        ...
        tariff_percent: Number.isFinite(calc.tariffAllAnnual) && calc.tariffAllAnnual > 0
            ? Math.round(calc.tariffAllAnnual * 10000) / 100
            : null,
        program: 'Страхование по подписке. Сбер Страхование Жизни',
        risks: calc.risks,
    };
}
