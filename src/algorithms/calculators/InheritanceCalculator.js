...
const INHERITANCE_GOAL_TYPE_ID = 11;

class InheritanceCalculator extends BaseCalculator {
    async calculate(goal, context) {
        ...
        if (!portfolio) {
            throw new Error(`Inheritance portfolio not found for class ${goal.goal_type_id}`);
        }

        const yieldOptions = { yieldMode: 'iszh_survival_matrix' };
        const {
            weightedYieldAnnual,
            initial_instruments,
            monthly_instruments,
            pdsProductId,
            insurance_risks: insuranceRisksFromYield
        } = await this.calculateWeightedYield(
            portfolio,
            goal,
            productRepository,
            context.projectId,
        ...
        if (Array.isArray(insuranceRisksFromYield) && insuranceRisksFromYield.length > 0) {
            details.insurance_risks = insuranceRisksFromYield;
        }

        return {
            goal_id: goal.id,
            goal_type_id: Number(goal.goal_type_id) || INHERITANCE_GOAL_TYPE_ID,
            goal_type: 'INHERITANCE',
            summary: {
                status: 'OK',
...
module.exports = new InheritanceCalculator();
