...
- **`summary/buildSummaryPdfLayoutModel.js`** — JSON для фронта/PDF без фиксированной обрезки: `goals_continuation`, `capital_distribution`, `layout_hints`. У карточки: **`goal_card_image`** — `repo_relative_path`, **`r2_object_key`** (`pdf-report-goal-cards/…`), **`public_url`** (если в env задан R2 public base и файлы залиты в R2). Заливка: **`npm run seed:pdf-goal-cards-r2`**. В ответе отчёта: **`pdf_summary_layout`** (`reportService.getClientReportData`).
- **Настройки агента** — таблица `agent_report_pdf_settings`.
- **API для ЛК** — `GET/PATCH /api/pfp/pdf-settings` (в ответе всегда есть `editor_schema`, **`cover_layout`**, **`summary_layout`**, **`goal_card_assets`** — манифест иллюстраций карточек целей с `public_url` для превью макета; в ЛК не редактируется). Загрузка фона в **Cloudflare R2**: `POST /api/pfp/pdf-settings/cover-background` (`image`, до 8 МБ). Превью: `GET /api/pfp/pdf-settings/cover-image` (прямой или signed URL). Переменные: `docs/env-cloudflare-r2.md`.
