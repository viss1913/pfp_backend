...
- История **уже пишется** в БД: таблица `ai_b2c_chat_ai_messages` (пары user/assistant при `POST /api/my/ai-b2c/chat_AI/stream`).
- **Список клиентов** `GET /api/pfp/clients` в каждом элементе добавляет поле **`chat_ai_messages`** (хронология, последние N сообщений на клиента). Пустой массив, если диалога не было — клиент без расчёта всё равно в списке, как и раньше.
- Query: **`include_chat_ai=false`** или **`0`** — не тянуть диалоги (легче ответ). **`chat_ai_limit`** — лимит на клиента в списке (по умолчанию 200, макс. 500).
- **`GET /api/client/:id`** — то же поле `chat_ai_messages` (по умолчанию до 500 сообщений, макс. 2000). Маршрут **`GET /api/pfp/clients/:id/plans`** по умолчанию **не** подмешивает чат; можно принудительно: `include_chat_ai=true`.
