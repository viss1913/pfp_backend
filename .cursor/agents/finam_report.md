...
### 6) Спейсер и `splitFinamPage4IntoStandalonePages` (PDF)

- В двухстраничных goal-HTML часто задано `article.page:first-of-type .spacer { flex: 0 0 8px }` (плотная первая страница в **одном** файле с двумя `article`).
- После нарезки на отдельные документы для Puppeteer в **каждом** фрагменте остаётся **один** `article` — он всегда совпадает с `:first-of-type`, спейсер не растягивается, футер «плывёт» вверх, снизу «дыра» с клеткой.
- В проде это компенсируется инъекцией в [`injectReportPdfPageFillA4.js`](../src/utils/injectReportPdfPageFillA4.js) (`article.page .spacer { flex: 1 1 auto !important; … }`). Не убирать без замены стратегии (класс на листе, отказ от `:first-of-type` и т.д.).
