...
| `src/services/reportPdfService.js` | Сборка списка HTML-страниц → Puppeteer **`page.pdf()`**; опция **`brandingAgentId`** (ЛК клиента) или **`agentId`** из JWT (агент). Для Finam v2 не применять v1 `injectReportPdfPageFillA4` и общий DejaVu font override; дефолт рендера v2 — `FINAM_REPORT_V2_RENDER_CONCURRENCY=1` (можно переопределить env). |
