...
| `src/reports/finam_v2/` | Finam Report v2: `buildFinamReportV2HtmlPackage.js` для PDF/API при `report_finam=2`, template-driven production composer (`finamV2PageManifest.js`, `finamV2TemplateLoader.js`, `finamV2TemplateAppliers.js`, `finamV2PageComposer.js`), хвостовые блоки Comon/ДУ/спецпредложения/инфляция/риски/подробный план, goal-страницы включая `page-goal-pension-v2.html` (3 листа), `page-goal-passive-income-v2.html`, `page-goal-save-grow-v2.html` и `page-goal-other-v2.html` (по 2 листа), preview, order, `page-wow-shared.css`, `finamReportV2Contract.js`, экспериментальный `buildFinamReportV2Html.js` |
| `src/services/reportPdfService.js` | Сборка списка HTML-страниц → Puppeteer **`page.pdf()`**; опция **`brandingAgentId`** (ЛК клиента) или **`agentId`** из JWT (агент). Для Finam v2 не применять v1 `injectReportPdfPageFillA4` и общий DejaVu font override; physical pages уже A4-холст `794×1123` из `finamV2TemplateLoader.js` с CSS scale шаблона `595×842`, поэтому Puppeteer `scale=1` фиксированно (без `FINAM_REPORT_V2_PDF_SCALE`); дефолт рендера v2 — `FINAM_REPORT_V2_RENDER_CONCURRENCY=1` (можно переопределить env). |
| `src/reports/finam/reportVersionResolver.js` | Resolver версии Финам-отчёта: опционально env `FINAM_REPORT_VERSION` (+ `FINAM_REPORT_VERSION_PROJECT_IDS`); иначе project-scoped `system_settings.report_finam`; Ростех и не-Finam — v1 |
